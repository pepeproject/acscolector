...
clean:
	./mvnw clean
	rm -f dists/pepe-acscollector-${RPM_VER}*.rpm
...
dist: api
    ...
    cp -v pepe-acscollector-${VERSION}-SNAPSHOT.jar lib/pepe-acscollector.jar && \
    fpm -s dir \
        ...
        --vendor 'Globo.com' \
        --description "Pepe ACS Collector service" \
        --after-install scripts/postinstall \
        -f -p ../dists/pepe-acscollector-${RPM_VER}.el7.noarch.rpm lib/=/opt/pepe/lib/ scripts/=/opt/pepe/scripts/ conf/=/opt/pepe/conf/ initscript=/etc/init.d/pepe && \
    cd $$old
